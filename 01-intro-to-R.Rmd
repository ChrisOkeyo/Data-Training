...
date: "The World Bank | [WB Github](https://github.com/worldbank) <br> December 2022"
output:
...
- Every session has two TAs. For this session, our TAs are __Christine Okeyo__ and __Benson Njogu__

- The TAs will help you troubleshooting __particular issues__ which make you unable to follow along the presentation.
...
- If you have a question feel free to share
...
These training sessions will offer an introduction to R and its amazing features

## About this session

This second session will present the basic concepts you will need to use R.
...
1. Go to the OSF page of the course (https://osf.io/86g3b/) and download the file located in `R for Stata Users - 2022 November` > `Data` > `whr_panel.csv`
...
* __You can put anything into an object__ : scalars, strings, datasets, vectors, plots, functions, etc.
...
* __Data frames:__ a combination of different vectors of the same length (the same as your dataset in Excel)
...
The `whr` is a data frame. You can also construct a new data frame from scratch by __combining vectors with the same number of elements__.
...
R has different kinds of data that can be recorded inside objects. The main types are string, integer and numeric, factors, and boolean.
...
## Integer and numeric
As default, R stores numbers in the numeric format (double).
...
### Exercise 4: Create a vector of strings <font size="5">(`r fa("clock")` 2 min)</font> 
...
### Exercise 5: Concatenate strings <font size="5">(`r fa("clock")` 3 min)</font> 
...
Factors are __numeric categorical values with text labels__. Turning strings into factors makes it easier to run different analyses on them and also uses less space in your memory.
...
### Exercise 7 <font size="5">(`r fa("clock")` 3 min)</font> 
...
Did you notice anything strange? That happens because the default way R stores numbers is *numeric*
---
